...
import { ITEMS, ITEMS_BY_SLOT, RARITY_META, RARITY_ORDER, SLOT_LABEL, SLOT_ORDER } from '@/data/items'
...
export default function Inventory() {
  ...
  const items = useMemo(() => {
    ...
    return pool
      .filter((item) => showLocked || ownedMap.has(item.id))
      .sort((a, b) => {
    ...
    <Screen title="Inventory" subtitle={`${collection.owned} of ${collection.total} collected`} back="/forge">
      <div className="space-y-4">
        <Card>
          ...
          <ul className="grid grid-cols-5 gap-1.5 mt-3">
            {RARITY_ORDER.map((rarity) => (
              <li key={rarity} className="text-center">
                ...
                <span className="block text-sm tabular text-parchment">
                  {collection.byRarity[rarity].owned}/{collection.byRarity[rarity].total}
                </span>
              </li>
            ))}
          </ul>
